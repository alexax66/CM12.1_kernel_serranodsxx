...
struct workqueue_struct *suspend_work_queue_pws;
...
#endif
			state = new_state;
			queue_work(suspend_work_queue_pws, &power_suspend_work);
		} else if (state == POWER_SUSPEND_ACTIVE && new_state == POWER_SUSPEND_INACTIVE) {
#ifdef CONFIG_POWERSUSPEND_DEBUG
			pr_info("[POWERSUSPEND] state deactivated.\n");
#endif
			state = new_state;
			queue_work(suspend_work_queue_pws, &power_resume_work);
		}
		spin_unlock_irqrestore(&state_lock, irqflags);
...
static int __init power_suspend_init(void)
{
	...
	suspend_work_queue_pws = create_singlethread_workqueue("p-suspend");

	if (suspend_work_queue_pws == NULL) {
		return -ENOMEM;
	}
...
static void __exit power_suspend_exit(void)
{
	...
	destroy_workqueue(suspend_work_queue_pws);
} 
