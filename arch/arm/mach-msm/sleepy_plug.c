...
static void __ref set_cpus(int n_cpus_on_requested) {
...
static void sleepy_plug_work_fn(struct work_struct *work)
{
#ifdef DEBUG_SLEEPY_PLUG
	pr_info("decision: %d\n",decision);
#endif
...
static ssize_t __ref sleepy_plug_active_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) {
